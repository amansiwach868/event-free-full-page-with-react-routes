...
import { default as commenterImg, default as commenterImg1 } from '../assets/images/png/commenter-img-1.png';
import calanderImg from '../assets/images/svg/calendar-img.svg';
import { commentData, feedUserData } from '../utils/helper.js';
import HeadingPara from './common/HeadingPara';

const NewPosts = () => {
    const [dropdown, setdropdown] = useState(0);
    const [inputValue, setInputValue] = useState(feedUserData.map(() => ''));
    const [comment, setComment] = useState(
        feedUserData.map(() => [...commentData])
    );
    const [likes, setLikes] = useState(
        feedUserData.map(() => (false))
    );
    const toggleDropdown = (i) => {
        setdropdown(dropdown === i ? null : i);
    }
    const addComment = (e, postIndex) => {
        e.preventDefault();

        const trimmedComment = inputValue[postIndex].trim();
        if (trimmedComment === '') {
        ...
        <div key={i} className="p-[35px] rounded-[30px] shadow-[0px_8px_35px_0px_#0000001A]">
                    ...
                    <div className="flex gap-10 items-center mt-[26px] max-xl:hidden">
                        <div className="flex gap-2.5">
                            <svg onClick={() => {
                                ...
                                newLike[i] = !newLike[i];
                                setLikes(newLike);
                            }} className={`cursor-pointer hover:scale-110 hover:fill-red-500 duration-300 ${likes[i] ? 'fill-red-500' : '!fill-transparent'}`} width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path className={`${likes[i] ? 'fill-red-500' : '!fill-transparent'}`} d="M7.5 4C4.4625 4 2 6.4625 2 9.5C2 15 8.5 20 12 21.163C15.5 20 22 15 22 9.5C22 6.4625 19.5375 4 16.5 4C14.64 4 12.995 4.9235 12 6.337C11.4928 5.6146 10.8191 5.02505 10.0358 4.61824C9.25245 4.21144 8.38265 3.99938 7.5 4Z" stroke="black" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                            </svg>
                            <p className='cursor-pointer sm:text-[16px] text-[14px] text-[#666666] font-normal'>{likes[i] ? data.postlikes + 1 : data.postlikes}</p>
                        </div>
                        <div className="flex gap-2.5">
                            ...
                            <p className='cursor-pointer sm:text-[16px] text-[14px] text-[#666666] font-normal'>{comment[i].length}</p>
                        </div>
                    </div>
                    {likes}
                </div>
            ...
            <div className={`grid duration-300 ${dropdown === i ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'} overflow-hidden mt-[26px]`}>
                <div className="overflow-hidden">
                    <form onSubmit={(e) => { addComment(e, i) }} className="w-full border-[#9CA0BE] border rounded-[35px] p-[7px] flex max-sm:flex-col gap-y-5">
                        <img className='min-w-[55px] max-w-[55px] h-[55px] rounded-full mx-auto' src={commenterImg} alt="commenterImg" />
                        <input value={inputValue[i]} onChange={(e) => {
                            const newInputValues = [...inputValue];
                            newInputValues[i] = e.target.value;
                            setInputValue(newInputValues);
                        }} type="text" required placeholder='Write your comment here...' className='w-full sm:mx-[21px] outline-0 input max-sm:text-center text-[15px] sm:text-[20px] font-normal' />
                        <button className="text-nowrap px-7 py-[11px] rounded-[50px] font-semibold bg-[#2C49FE] text-white hover:bg-[#01C8FF] duration-300 cursor-pointer">Post Comment</button>
                    </form>
                    {comment[i].length > 0 ? (<div className="flex gap-6 flex-col w-full mt-[34px]">
                        {comment[i].map((commantData, index) => (

                            <div key={index} className="flex max-sm:flex-col items-start gap-5">
                                ...
                                <div className="w-full">
                                    ...
                                    <div onClick={() => deleteComment(i, index)} className="flex cursor-pointer gap-[5px] mt-2 ">
                        ...
                        <p className=' text-black/60 text-xl text-center mt-4'>No comment yet</p>
                    )}
                </div>
            </div>
        </div>
